...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{Zusammenfassung Analysis HS 2013, FS 2014}
...
Sei $f: [1,\infty) \to \R_{+}$ monoton fallend. Dann konvergiert $\sum^{\infty}_{k=1}{f(k)}$ genau dann, wenn $\int^{\infty}_{1}{f(x)dx}$ existiert.
...
\begin{enumerate}[label={(}\arabic*{)}]
 ...
 \item $\int_{x_0}^{x_1}f(x)dx := F(x_1) - F(x_0)$ heisst Integral von $f$ über $[x_0, x_1]$.
   Dabei ist $a \leq x_0 \leq x_1 \leq b$ und $F' = f$.
 \item \textbf{Hauptsatz:} $F(x) \defeq \int_{x}^{b}f(x)dx, \  x \in [a,b] \implies F' = f.$
\end{enumerate}
...
\emph{Bemerke:} Es kann zusätzliche, konstante Lösungen für $y$ geben, nämlich für alle $y$ mit $g(y) = 0$.
...
\paragraph{Ansatz vom Typ 'rechte Seite'} beschreibt die Idee, das die Störfunktion eine ähnliche Gestalt wie die spezielle Lösung hat. \textbf{Beispiel:}
\[ y'' + y' -6y = 3e^{-4x}\]
Die dazugehörige homogene DGL ist $y''+y'-6y=0$, das charakteristische Polynom $p(\lambda)=\lambda^2+\lambda-6=0$ mit $\lambda_{1,2}=2,-3$. Die allgemeine Lösung der homogenen DGL ist $y(x)=c_1e^{-3x}+c_2e^{2x}$. Der Ansatz:
 \[y_s(x)=Ke^{-4x}\]
...
\paragraph{Test für Diff'barkeit}

$f$ differenzierbar in $x_0 \implies \frac{\partial f}{\partial x^1}(x_0)$ existiert und das Differential $\mathrm{d}_{x_0}f = \left(\frac{\partial f}{\partial x^1}(x_0), \dots, \frac{\partial f}{\partial x^n}(x_0) \right)$ \\
$f$ ist total differenzierbar, wenn die partiellen Ableitungen stetig sind. ($f\in C' \implies f$ differenzierbar).
...
\subsubsection{Definitionen}
\begin{enumerate}[label={(}\arabic*{)}]
\item $\lim_{\substack{x \to {x_0} \\ x\not  = {x_0}}}{\frac{{f\left( x \right) - f\left( {{x_0}} \right)}}{{x - {x_0}}}}$
\item $\frac{{\partial f}}{{\partial {x^i}}}({x_0}) \defeq  \lim_{\substack{h \to 0 \\ h\not = 0}}{\frac{{f({x_0} + h{e_i}) - f({x_0})}}{h}}$
\end{enumerate}
\subsubsection{Rechenregeln}
\begin{enumerate}[label={(}\arabic*{)}]
...
\begin{enumerate}[label={(}\arabic*{)}]
 ...
 \item $\int |f(x)|dx \geq |\int f(x)dx|$
\end{enumerate}
\subsubsection{Polynome und Wurzeln}
\begin{enumerate}[label={(}\arabic*{)}]
...
\subsubsection{Ansatz vom Typ rechte Seite}
\begin{tabular}[m]{c|c}
Störfunktion $q(x)$ & Ansatz für $y_p(x)$ \\[2px] \hline
$ae^{\mu x}$ & $be^{\mu x}$ \\[2px] 
$a\sin{vx}$ od $b\cos{vx}$ & $c\sin{vx} + d\cos{vx}$ \\[2px] 
$ae^{\mu x}\sin{vx}$ od $be^{\mu x}\cos{vx}$ & $e^{\mu x}(c\sin{vx} + d\cos{vx})$ \\[2px] 
$P_n(x)e^{\mu x}$ & $R_n(x)e^{\mu x}$ \\[2px] 
$P_n(x)e^{\mu x}\sin{vx}$ od $Q_n(x)e^{\mu x}\cos{vx}$ & $e^{\mu x}(R_n(x)\sin{vx} + S_n(x)\cos{vx})$ \\[2px] \hline
\end{tabular}
